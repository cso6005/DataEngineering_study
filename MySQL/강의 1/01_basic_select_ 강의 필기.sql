...
select 컬럼명, 컬럼명 [, .....]  => 조회할 컬럼 지정.(만들때 순서랑은 상관없)  --  *: 모든 컬럼
...
-- EMP 테이블의 모든 컬럼의 모든 항목을 조회. -- 한줄에 해도 여러 줄에 해도 되고 들여쓰기해도 되고 알아서
...
-- EMP 테이블의 업무(job) 어떤 값들로 구성되었는지 조회. - 동일한 값은 하나씩만 조회되도록 처리.(컬럼이 categorical 형식 명목형일 때 뭘 분류할 때. 생별, 혈액형, bool  -> 값이 n개로 결정되는.)
select distinct job
from   emp;

select distinct job, dept_name  -- 이때 job에 대한 중복값 아님. job과 dept_name 둘에 대한 중복값.
from   emp;

-- EMP 테이블의 부서명(dept_name)이 어떤 값들로 구성되었는지 조회 - 동일한 값은 하나씩만 조회되도록 처리.
select distinct dept_name
from   emp;  -- null도 값으로 표현되어 나온다.

-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회한다.
-- select 컬럼명 [as] 별칭 -> 컬럼명으로 조회한 것을 별칭으로 보여줘라. as는 생략가능. 이것도 가독성을 위해 생략하면 다 생략하고 넣으면 다 넣고
-- 별칭 : 공백 넣을 수 없다. 주의. 공백 주면 구문이 떨어진 것으로 인식한다. 만약 넣고 싶으면 " "큰따옴표로 감싸준다. 이렇게 들어가면 그냥 다 해주기. 가독성을 위해
-- 이렇게 큰따옴표(공백 주고 싶을 때) 작은 따옴표(문자열 값을 표현) 역할이 다르기에 주의해서 써야 함.
...
- 산술 연산자 
	+, -, *, /, 나머지 %, mod, 몫 div
- date/time/datetime 
    +, - : 마지막 항목(date:일, time: 초, datetime: 초) 의 값을 +/- 연산 => 계산 결과가 정수형!으로 반환된다. (ex:20100102) '2010-01-01' +1
- 여러개 값을 합쳐 문자열로 반환
	concat(값, 값, ...) 
...
select 10 as a, 20 as b;  -- select 절에 상수값을 넣어서 확인. 테이블이 있는 게 아니니 from 생략.
select '안녕하세요' as 인사말;
...
select concat('이름','홍길동'),
      concat(20,40),
	   concat('나이', 20);  -- 이렇게 정수와 문자열도 합칠 수 있다.
...
-- 연산은 그 컬럼의 모든 값들에 일률적으로 적용된다. salary면 그 해당 컬럼의 값 모두 연산 ==> 원소 별로 연산!
select emp_name,
       salary, 
       salary + 10000
from   emp;
...
/* *************************************
where 절을 이용한 행 선택 
구문 : where 컬럼 연산자 피연산자값   -- 컬럼의 값들과 연산했을 때 True 인 행들만 조회 =>> Filter 개념
...
-- EMP 테이블에서 커미션비율(comm_p0.2~0.3 사이인 직원ct)이 의 ID(emp_id), 이름(emp_name), 커미션비율(comm_pct)을 조회.
select emp_id,
	   emp_name,
       comm_pct
from   emp
...
select emp_id, emp_name
from   emp
where  emp_name like '%#%%' escape '#';  -- #뒤에 패턴문자가 오면 literal라고 정해주는 것. 어떤 거 뒤에 올지는 본인이 지정.


-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name)을 조회.
-- where dept_name = null  null은 없는 값. 없는 값은 비교할 수 있는 대상이 아니다. 그래서 아예 연산자를 만듦. 이렇게 해도 정상실행 되는 게 틀렸다는 거 주의하기.
...
-- not (where 조건)
-- not (dept_name is null) 이렇게 not 연산자를 쓰는 거 가능. 괄호는 상관없지만 해주는 게 좋음. not dept_name in ('Shipping')  
...
from emp
where job <> 'IT_PROG';
-- where job not in ('IT_PROG');


-- TODO: EMP 테이블에서 이름(emp_name)이 'Peter'인 직원들의 모든 컬럼의 데이터를 조회 -- 값은 대소문자 구분 해주기.
select *
from emp
...
-- 참고: date/datetime에서 년도만 추출: year(값). ex) year('2020-10-10') => 2020
-- hire_date는 date type이다. 
...
select emp_id, emp_name, job
from   emp
where  emp_id in (110, 120, 130); -- pk라 중복 없이 나올 것.
...
-- where salary >= 16667; --200000/12; 사실 함수나 연산자나 최대한 덜 쓰는 게 좋기때문에 계산할 수 있는 건 이렇게 적는 게 좋음.


/* *************************************
 WHERE 조건이 여러개인 경우

(조건) AND (조건) -> 참 and 참 -> 참: 조회 결과 행. 둘 다 참일 때만 조회 결과 행.
...
ex) where 참 or 거짓 and 거짓 ;  => 참 or (거짓) => 참
...
select emp_id, emp_name, job, dept_name
from  emp
where job like '%MAN%' and dept_name in ('Marketing', 'Sales'); -- 한 컴럼에 대해서니깐 in 연산자로 가능. 
...
/* *******************************************************************
order by를 이용한 정렬
- 컬럼 행의 한 속성 데이터만 정렬되는 건 아니다. 한 행 단위로 한 행 자체가 정렬된다.
- 워크벤치 클라이언트에서 표로 바꾸는 기능이 있긴 하지만 사실 sql은 명령어로 정렬 해줘야 한다.
...
- order by 정렬기준컬럼 정렬방식 [, ...]
    ...
    - 정렬기준컬럼 지정 단위: 컬럼이름, 컬럼의순번(select절의 선언 순서) 둘 중 아무거나.
     
      ex ) select salary, hire_date 
...
select emp_id, emp_name, job, salary
from emp
order by job asc, salary desc, emp_name desc; -- 1차로 job 오름차순 정렬, 2차로 salary 내림차순(job이 같은 것 안에서 정렬), 3차 name 내림차순


-- 부서명을 부서명(dept_name)의 오름차순으로 정렬해 조회하시오.
-- 오름차순하면 null 값인 애들은 제일 먼저 나온다. 제일 작은 걸로 판단
select * from emp
order by dept_name ; 
