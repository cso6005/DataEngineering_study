...
-- 테이블 drop 시 외래키 제약 조건 제거. 부모 테이블 drop 전에 
...
-- 부서 테이블
-- ------------------------------------------------------
DROP TABLE IF EXISTS dept; -- 실행하면 drop하고 다시 생성. 
-- 테이블 생성
CREATE TABLE dept(
    dept_id 	INT PRIMARY KEY, 
    dept_name 	VARCHAR(100) NOT NULL,
    loc 		VARCHAR(100) NOT NULL  -- 지사
);
-- 테이블 데이터 삽입
...
CREATE TABLE job(
    job_id 		VARCHAR(30) PRIMARY KEY, -- 참조될 수 있는 컬럼값
    job_title 	VARCHAR(100) NOT NULL, -- 업무명
    min_salary 	DECIMAL(6) NOT NULL,  -- 그 업무를 하는 직원의 최소 급여
    max_salary 	DECIMAL(6) NOT NULL   -- 그 업무를 하는 직원의 최대 급여
);
...
-- FK (Foreign key 컬럼): 부모 테이블의 pk 컬럼의 값들만 가질 수 있다.    없는 걸 어떻게 찾아. 
-- 						  FK 컬럼의 타입은 부모 테이블 pk 컬럼 (참조할 컬럼)의 타입과 동일하게 준다.!
CREATE TABLE emp(
    ...
    -- 제약조건명: 제약조건타입의 약자 _ 테이블_ 컬럼   # 이게 관례적
	-- fk_자_부모테이블  or  fk _자_컬럼_부모테이블(컬럼)   ????
    -- references 참조할 부모 테이블(참조할 컬럼명 pk)  
    
    -- ON 설정 : 부모 테이블 행 삭제했을 때 관련
    -- 부모 테이블이 지워지면 null로 설정한다. 부모가 지워지면 행 자체을 지우겠다. 두 가지 설정이 가능한데, 우리는 null 설정을 지금 하였다.
    -- null설정을 할 수 있는 것은 dept_id 가 int로 null 값을 가질 수 있기에/ 만약 자식테이블 fk 컬럼값이 null을 가질 수 없는 컬럼값이면 이 설정 못함.
    -- 만약 이 설정 안 하면 부모 테이블을 못 지운다. 에러 날 것.
    
    -- 사실 자식 테이블의 이름은 바꿔도 된다. 부모테이블의 pk 이름이랑 같을 필요는 없다.
...
-- 이 테이블은 다른 테이블과 관계성은 없다.
-- 특정 테이블  범위가지고 하는 거라 일대일로 매칭되는 fk 키가 없다.
-- 나중에 salary와 관계는 가지게 될 듯.
INSERT INTO salary_grade (grade,low_sal,high_sal) VALUES (1, 0, 5000);  -- 0  ~ 5000 사이가 1등급
...
-- <테이블 합치는 것에 의미 >
-- 데이터가 분산시켜 두었는데 조회 시 select 시 여러 테이블에서 데이터를 들고올 때 하나의 sql문으로 처리되어야 한다.
-- 이를 위해서 join 문. -> 세 테이블을 합쳐서 합친 것을 from 하면 된다. 
-- 근데 합치는데 어떤 테이블은 30행 어떤테이블은 107행 어떤 테이블은 50 행 이라면 어떻게 합치냐.

-- emp_id 100인 직원의 모든 정보
-- emp의 직업id는 직원 테이블에서 직원 id 100인 거랑 합쳐줘야 하고, emp는 부서 테이블의 부서의 값이 90인 행이랑 합쳐져야 한다.
-- 아무거나 붙이는 게 아니라 이렇게 관계가 있는 데이터끼리 붙어야 하는 것.
-- 부모 테이블의 직업 id와 자식 테이블의 직업 id의 값이 같은 것 끼리 합쳐야 하는 것.
